...
function makeTable(params: TMakeTableParams): HTMLTableElement  {
	const totalColumns = params.headers.length,
		myICss = new iCss(),
		colWidths: number[] = [],
		testWidthElement: HTMLSpanElement = document.createElement("span"),
		defaultCaptionTitleStyling: ElementCssProperties = {
			color: "lime",
		...
		defaultCaptionSubtitleStyling: ElementCssProperties = {
			fontSize: "83%",
         color: "yellow",
         fontWeight: "normal"
		};
   ...
   let i: number,
		...
		captionElem: HTMLTableCaptionElement,
		pNode: HTMLParagraphElement,
     //   colgrpElem: HTMLTableColElement,
        colElem: HTMLTableColElement,
        colStyle: string,
    //    style: any,
		value: any, // string | {iValue?: any; wrapLink: any} | {},
		headerTitle: string,
//		matches: RegExpMatchArray | null,
		tableOptions: TMakeTableOptions | null = null,
		rowCounter = 0;
...
//		tblElem.appendChild(colgrpElem);
		...
		//			if (col.hasOwnProperty(style))
						colStyle += `${style}:${col[style as keyof ElementCssProperties]}`;
...
// this attaches the table to the existing node in the document
	if (!params.attach) {
		...
		throw new Error(
			`an invalid parent/attachment DOM element was passed to makeTable(): ${params.attach}`
		);
	}
   ...
   tblElem.appendChild(trElem);
    ...
    // add the column for row counting
      ...
      // rest of headers
		tdNode = document.createElement("th");
		trElem.appendChild(tdNode);
		headerTitle = typeof params.headers[i] == "string" ? params.headers[i] as string : 
				(params.headers[i] as EnhancedHeader).title;
		if (typeof params.headers[i] != "string" && (params.headers[i] as EnhancedHeader).style)
			tdNode.setAttribute("style", (params.headers[i] as EnhancedHeader).style!);
...
//						myICss.adjustWordStyleForTableColumn(value.textContent, tdNode, testWidthElement);
					tdNode.replaceChild(value, tdNode.firstChild as ChildNode);
				}
			}
	}
...
function setElementCssProperties(attribs: ElementCssProperties, styleProperty: CSSStyleDeclaration) {
	...
	if (attribs.fontWeight)
		styleProperty.fontWeight = attribs.fontWeight as string;
	if (attribs.color)
...
/*
function downloadProcess(fileContent: BlobPart[], savedFileName: string, fileType: "html" | "csv" | "xml" | "aspx") {
...
/*
  ...
  //  savedFileName = savedFileName.replace(/ /g, "%20");
	link.setAttribute('href', url);
	link.setAttribute('download', savedFileName);
    link.onclick = (event: Event) => {
        let anchorElem: HTMLAnchorElement = event.currentTarget as HTMLAnchorElement;
